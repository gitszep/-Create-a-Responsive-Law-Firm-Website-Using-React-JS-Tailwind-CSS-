    ...
    <nav className='lg:hidden relative z-20'>
      ...
      >
        ...
        {navigation.map((item, index) => {
          return (
            <li key={index}>
              <Link to={item.href} className='hover:text-accent-hover transtion-all duration-300 cursor-pointer' spy={true} offset={-200} activeClass="active" smooth={true} duration={500}>
                {item.name}
              </Link>
            </li>
          );
